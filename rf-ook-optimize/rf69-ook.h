...
class RF69A : public RF69<SPI> {
  public:
    ...
    void setSpecial(uint32_t special);
...
template< typename SPI >
uint8_t RF69A<SPI>::readRSSI2() {
  uint8_t rssi = 0;
   if (true)
   {
     //RSSI trigger not needed if DAGC is in continuous mode
     this->writeReg(REG_RSSICONFIG, RSSI_START);
     while ((this->readReg(REG_RSSICONFIG) & RSSI_DONE) == 0x00); // Wait for RSSI_Ready
   }
  rssi = this->readReg(REG_RSSIVALUE);
  //rssi >>= 1;
  return rssi;
...
template< typename SPI >
void RF69A<SPI>::setSpecial(uint32_t special) {
	if (special == 0) {
		//Low M
	  this->writeReg(0x0B, 0x20);
	  this->writeReg(0x58, 0x1B);
	  this->writeReg(0x6F, 0x20);
	  this->writeReg(0x71, 0x06);
	} else if (special == 1) {
		//High M
	  this->writeReg(0x0B, 0x00);
	  this->writeReg(0x58, 0x1B);
	  this->writeReg(0x6F, 0x30);
	  this->writeReg(0x71, 0x00);
	} else if (special == 2) {
		//Low M, SensBoost
	  this->writeReg(0x0B, 0x20);
	  this->writeReg(0x58, 0x2D);
	  this->writeReg(0x6F, 0x20);
	  this->writeReg(0x71, 0x06);
	} else if (special == 3) {
		//High M, SensBoost
	  ...
	  this->writeReg(0x71, 0x00);
		...
		//Low M, SensBoost, DAGC=0
	  this->writeReg(0x0B, 0x20);
	  this->writeReg(0x58, 0x2D);
	  this->writeReg(0x6F, 0x00);
	  this->writeReg(0x71, 0x06);
	} else if (special == 7) {
		//High M, SensBoost, DAGC=0
	  this->writeReg(0x0B, 0x00);
	  this->writeReg(0x58, 0x2D);
	  this->writeReg(0x6F, 0x00);
	  this->writeReg(0x71, 0x00);
