...
void ReceiveSensitivity(class DecodeOOK* decoder);
...
PowerTestDecoder       ptest(   45, "PTEST", ReceiveSensitivity);
//
void setupDecoders() {
	//   decoders[di++] = &emx;
	//decoders[di++] = &fsx;
	...
	while(true) {
			...
			sd = 2;
				for ( specialmode = 0; specialmode < 8; specialmode++) {
					...
					printf("scan br%d, BW%d, FL%d, THD%d, SM%d\r\n", bitrate, bw, fl, fixthd, specialmode);
					receiveOOK(bitrate, fl, bw, sd);
				}
		printf("Finished full loop\r\n");
	}
...
void BytesShiftLeft(uint8_t * arr, uint8_t len, uint8_t n)
{
    uint8_t m = (n >> 3); //number of integral bytes
	n = n & 0x03;
	for (uint8_t i = 0;  i < len - m - 1;  ++i)
    {
        arr[i] = (arr[i+m] << n) | (arr[i+m+1] >> (8-n));
    }
    arr[len-m-1] = arr[len-1] << n;
	for (uint8_t i = len - m;  i < len;  ++i)
	{
		arr[i] = 0;
	}
}
...
void ReceiveSensitivity(class DecodeOOK* decoder) {
	uint8_t pos;
	uint8_t* data = (uint8_t*) decoder->getData(pos);

	static uint32_t last_print = 0;
	static uint8_t mesgcnt = 1;
	uint32_t now = millis();
	printf("BR,%d, BW,%d, FL,%d, SD,%d, THD,%d, NS,%d, SM,%d, ", bitrate, bw, fl, sd, fixthd, g_rssiavg, specialmode);
	...
	//remove last bit of preamble
	BytesShiftLeft(data, pos, 1);
	uint8_t idx = 0;
	uint8_t parity_errors = 0;
	uint16_t counter = ((uint16_t)data[idx]) << 8;
	if (parity_even_bit(data[idx]) == ((data[idx+1] & 0x80) == 0)) parity_errors++;
	idx++;
	BytesShiftLeft(data+idx, pos-idx, 1);
	counter |= data[idx];
	if (parity_even_bit(data[idx]) == ((data[idx+1] & 0x80) == 0)) parity_errors++;
	//printf("count=%d\r\n", counter);
	idx++;
	BytesShiftLeft(data+idx, pos-idx, 1);
	uint8_t power = data[idx];
	if (parity_even_bit(data[idx]) == ((data[idx+1] & 0x80) == 0)) parity_errors++;
	//printf("power=%d\r\n", power);
	idx++;
	BytesShiftLeft(data+idx, pos-idx, 1);
	uint8_t unused = data[idx];
	if (parity_even_bit(data[idx]) == ((data[idx+1] & 0x80) == 0)) parity_errors++;
	//printf("unused=%d\r\n", unused);
	idx++;
	BytesShiftLeft(data+idx, pos-idx, 1);
	uint8_t sum = /*6 +*/ (counter >> 8) + counter + power + unused;
	uint8_t checksum = sum == data[idx] ? 0 : 1;
	//printf("SUM %s\r\n", sum == data[idx] ? "OK" : "NOK");
	printf(", N, %d, P, %d, U, %d, e, %d, s, %d, %s,", counter, power, unused, parity_errors, checksum, (parity_errors + checksum) == 0 ? " OK" : "NOK");
	//printf("\r\n");
	printRSSI();
	printf("\r\n");
...
void receiveOOK(uint32_t br, uint8_t fl, uint8_t bw, uint8_t sdf) {
	...
	//trick to toggle to sleep and receive
	//hopefully it clears effects of DAGC=0 change.
	rfa.DataModule(0x68);
